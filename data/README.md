...
RobustVGGTの公開scoreを使い，TartanAirの4画像系列で入力集合の構成を変えた比較の集計である．各系列について，別environmentのdistractorを4枚含む条件と，その4枚を同一sceneの冗長な画像へ置換した条件を保存する．正しい画像の保持判定，同一forwardのscoreを再正規化した反実仮想，coreから見えずconnectorから見える表面の3D completenessを含む．画像やdataset本体は含まない．
...
4画像系列の各々について，4枠を別scene画像または同一sceneの冗長画像へ置き換えた全16組合せ，合計64回のVGGT推論を集計したデータである．遠方側の正しい2画像が保持された組合せ，同じ枚数でのscoreの幅，各外れ画像候補の対応づけた効果を含む．また，各推論の内部表現を固定したまま，別scene画像をscoreの尺度決めからだけ除いた反実仮想も含む．画像やdataset本体は含まない．

## `shared_output_projectivity.json`

同じ正しい8画像だけを入力した条件，同一入力の再実行，別scene画像4枚を追加した条件，同一sceneの冗長画像4枚を追加した条件を，TartanAirの4画像系列で比較した集計である．中心側5画像だけでglobal Sim(3)を合わせた後の共通camera，depth，point mapの変位と，ground truthに対する共通point map誤差を含む．4条件×4画像系列の16推論であり，画像やdataset本体は含まない．
...
- 外れ画像部分集合: 上と同じ4画像系列に対する各16組合せ．64回の推論や組合せを独立標本とは数えず，画像系列ごとの応答曲線として扱う．
