...
## `one_world_vggt.json` / `one_world_vggt_qualitative.npz`

VGGTが出すcamera，depth，3D点，追跡結果の相互整合性を，同じsensor noise模様を反復する入力と，毎回異なるnoise模様を使う入力で比較した集計である．TartanAirの4画像系列，撮影位置2水準，noise条件2水準，3 seedからなる48回の推論を含む．JSONには対応づけた条件差，既知答えcontrol，正解上で可視な点だけに限定した追加監査，撮影位置構成の事後比較を保存する．定性的な実例は，正解位置と三つの予測位置がすべて画像端から5%以上離れた候補から選んだ．JSONではモデル入力座標と元画像座標の距離を区別し，NPZには512 × 384へ縮小した画像と同じ縮小座標上の四位置を保存する．元のTartanAir画像，depth，model checkpointは含まない．

## `recal3r_single_write_screen.json` / `recal3r_single_write_qualitative.npz`

ReCal3Rの一回のscene-state書き込みだけを止め，次の等間隔抽出画像における奥行きと位置合わせ後3D点の変化を測った予備実験の公開用集計である．TUM RGB-D Freiburg3 walkingの4系列，各2介入，計8介入時点を含む．JSONには固定条件，カメラを除いた介入時点別結果，系列単位の凍結集計，検証用比較，同じ処理系での再実行一致，主張境界を保存する．NPZには事後選択した一例の縮小RGB，正解・予測奥行き，画素別AbsRelを保存する．公開ファイルと図の再生成にはモデルコードや生の実験出力を必要としない．

## `counterworld_stage0_summary.json` / `counterworld_stage0_qualitative.npz`

同じ光学中心から回転だけを変えて撮影すると同じ画像を作る，近距離・遠距離の球について，二端点と連続区間の解析的関係，計測器の事前検査64場面，還元の診断256場面，二回の無効な実行をまとめた公開用集計である．二回目の実行は768レンダラ比較中16比較のRGB誤差が事前上限を越えたため，256場面の診断を科学的証拠として解釈しない．NPZは，固定した256場面で最初に保存された有効な一場面の128×128 RGB配列4枚を含み，問題設定の図示だけに使う．
...
- VGGT出力間整合性: TartanAir 4画像系列．48回の推論は，撮影位置構成，noise条件，seedを変えた同じ4系列への反復測定である．方向の判定は各系列内で3 seedの中央値を取り，独立単位は4画像系列とする．
- ReCal3R一回書き込み介入: TUM RGB-D Freiburg3 walking 4系列．各系列の2介入は反復測定であり，独立単位は4系列とする．
- Counter-World Stage 0: 計測器の事前検査64場面と還元の診断256場面である．レンダラ比較や画素を独立場面とは数えない．二回目の実行は計測器の検査が不合格であり，256場面の成功診断を経験的な成功率として扱わない．
