...
現在の判断は，実験候補としては残すが，独立テーマとして着手する段階ではない，というものである．最初に必要な条件を揃えた比較は実装負荷が高く，成功の根拠となる予備結果もない．成功すれば主要国際会議で強い評価を狙える手法になり得るが，幅広い条件への一般化と実用上の帰結は別の検証として残る．
...
この場合，「関係式を予測すると最終3Dが良くなる」という主張はできない．初期整合性，収束速度，特定のdegeneracyに限った現象として再度先行研究と論文上限を評価する必要がある．

### Q8．現時点でこのテーマを実装すべきか

**A．候補として保存する価値はあるが，独立テーマとして直ちに大規模実装する根拠はない．**

同一の出力形式を持つ先行研究は未発見であり，仮説を判定する実験も明確である．一方，予備結果がなく，強い座標比較が同等になる可能性が高い．さらに，最初の実験が成功しても，幅広い条件への一般化と実用上の帰結は別に示す必要がある．したがって，現時点では高コストで成功確率の読めない候補として扱う．
...
| 最終3Dは同じだが，初期整合性，conditioning，収束速度だけが良い | 仮説を縮小する | Optimization characterizationとして先行研究と論文上限を再評価する |
...
## 9．今回のtheme-searchから得た知見

1. **新しさの中心はsolverではなく，modelの出力契約である．** Global optimizationやconstraint solvingをfeed-forward 3Dへ組み合わせること自体は既に広く行われている．
2. **最も強い比較対象は既存の大規模modelではなく，同じ座標を同じ関係式へ変換する内部controlである．** このcontrolなしでは，精度差が表現，solver，capacityのどこから来たか分からない．
3. **四次元nullspaceは正しさの証明にならない．** 誤った対応でも一貫していれば，きれいなnullspaceを作れる．代数的な診断だけを主要結果にしてはならない．
4. **対応関係を選ぶ部分は，関係式以外の経路で大きな情報を運べる．** 比較時にはモデル構成だけでなく，実際に選ばれたcameraとpixelの組まで同一にする必要がある．
5. **失敗してもすぐにcharacterization paperになるわけではない．** 通常sceneで広く再現する数値条件や階数が崩れる条件の法則が得られない限り，残るのは実験protocolまたは既存テーマの一つのablationである．
6. **一つの成功結果だけではoral級の主張にならない．** 最初の条件を揃えた比較は，想定した機構，通常の3D再構成結果，強い比較対象の三点を一度に検証できるが，幅広い条件への一般化と実用上の帰結は別の独立した証拠である．
